...
},{"./helpers":6,"deep-equal":1}],5:[function(require,module,exports){
var equalsOptions = { strict: true };
var _equals = require('deep-equal');
var areEquals = function (a, b) {
    return _equals(a, b, equalsOptions);
...
},{"./core":4,"./helpers":6,"deep-equal":1}],6:[function(require,module,exports){
...
FSBL.addEventListener('onReady', function () {
	...
	//SUB to SDTOPIC AND GET SNAPSHOTS
	/*FSBL.Clients.RouterClient.subscribe("SDtopic", function(error, notify) {
		if (error) {
	...
	var x,y;
	// listen to data in a listen/transmit method, main listner
	 FSBL.Clients.RouterClient.addListener("SDFINSEMBLECHANNELa", function(error,response) {
		if (error) {
			console.log("SDFINSEMBLECHANNEL Error: " + JSON.stringify(error));

		} else {
			console.log("SDFINSEMBLECHANNEL Response: " + JSON.stringify(response));
			x=response;
            var table = document.createElement("p");

            table.innerHTML=JSON.stringify(x);
    ...
    FSBL.Clients.RouterClient.addListener("SDFINSEMBLECHANNELb", function(error,response) {
        if (error) {
            console.log("SDFINSEMBLECHANNEL Error: " + JSON.stringify(error));
        } else {
            ...
            y=response;
            console.log("patch "+y);
            toto=jsonpatch.apply_patch(toto,y)
            var table = document.getElementById("p");
            console.log("apres patch"+JSON.stringify(toto));
            table.innerHTML=JSON.stringify(toto);
